...
const ALL_WIDGETS = [
  ...
  { id: "list_fichajes", label: "├Ültimos fichajes", module: "fichajes", icon: Clock },
...
export default function DashboardPage() {
  ...
  // Estados para previsualizaci├│n PDF
  ...
  function isWidgetVisible(id: string) {
    ...
    // Si est├í en la config, usar su valor; si no est├í, mostrar por defecto (nuevo widget)
  ...
  async function loadAll() {
    try {
      ...
      // Cargar m├│dulos actualizados de la sesi├│n cada vez que cargamos todo
      const user = getUser();
      if (user) {
          ...
          <div className="bg-white rounded-xl p-4 md:p-6 shadow-sm border hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl md:text-3xl font-bold mt-1">{data.saldoTotal.toLocaleString('es-ES', {minimumFractionDigits: 2, maximumFractionDigits: 2})} Ôé¼</p>
              ...
              <div className="h-52 md:h-64 w-full min-h-[200px]">
                {hasModule("clientes") && data.stats && data.stats.topClientesSemana.length > 0 ? (
                  ...
                  <div className="h-full flex flex-col items-center justify-center text-slate-400 gap-2">
                    ...
                    <p className="text-xs">No hay datos suficientes para mostrar el gr├ífico</p>
                  </div>
                )}
              </div>
            </div>
                ...
                {data.trabajandoAhora.map((t) => (
                  <div key={t.id} className="p-3 md:p-4 hover:bg-gray-50 flex items-center justify-between">
                    <div>
                      ...
                      <div className="text-xs text-gray-500">{t.cliente_nombre || "Sin cliente"} ┬À Desde {hora(t.desde)}</div>
                    </div>
        ...
        {shouldShowWidget("list_fichajes", "fichajes") && (
          <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
            <div className="p-4 md:p-6 border-b">
              <h2 className="text-base md:text-lg font-semibold flex items-center gap-2"><Clock className="w-5 h-5 text-gray-400" /> ├Ültimos fichajes</h2>
            </div>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {data.ultimosFichajes.map((f) => (
                      <tr key={f.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 md:px-6 py-3 text-gray-500 hidden sm:table-cell">{f.cliente_nombre || "ÔÇö"}</td>
            ...
            {!data.facturasPendientesList.length ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                  <thead className="bg-gray-50/50 text-gray-500">
                    <tr>
                      <th className="px-4 md:px-6 py-3 font-medium">N├║mero</th>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {data.facturasPendientesList.map((f) => (
                      <tr key={f.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 md:px-6 py-3">{f.cliente_nombre || "ÔÇö"}</td>
                        ...
                        <td className="px-4 md:px-6 py-3 font-bold text-right">{Number(f.total).toLocaleString('es-ES', {minimumFractionDigits: 2})} Ôé¼</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
