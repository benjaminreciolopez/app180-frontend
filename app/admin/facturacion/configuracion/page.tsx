...
export default function ConfiguracionFacturacionPage() {
  ...
  // Factores de confianza del OCR para bloqueos dinámicos
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>, type: 'logo' | 'certificado' | 'migracion') => {
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        else endpoint = "/admin/facturacion/configuracion/emisor/upload-evidencia" // Nuevo endpoint sugerido o reusar lógica

        try {
          const res = await api.post(endpoint, { file: base64String, fileName: file.name })
          toast.success(type === 'logo' ? "Logo subido correctamente" : "Evidencia de migración guardada")
            ...
            toast.promise(
              api.post("/admin/facturacion/configuracion/emisor/ocr-migracion", { file: base64String }),
              {
                ...
                success: (ocrRes: any) => {
                  ...
                  return `Factura analizada. Campos bloqueados por alta fiabilidad: ${ext.numeracion.confidence > 0.9 ? 'Numeración' : ''} ${ext.economicos.confidence > 0.9 ? 'Económicos' : ''}`;
                },
                error: 'No se pudo extraer información automática, por favor rellena manualmente.'
              }
            );
          }
        } catch (err: any) {
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2 text-blue-600">
                  ...
                  <h3 className="font-bold">Certificado Digital (Firma Electrónica)</h3>
                </div>
                <p className="text-xs text-slate-500">
                  Este certificado se utiliza para firmar las facturas y garantizar la integridad de los envíos a la AEAT (Veri*Factu).
                </p>
                ...
                />
                <div className="border border-dashed rounded-lg p-4 bg-slate-50 flex flex-col gap-4">
                  <div className="flex items-start gap-3">
                    <div className={`p-2 rounded ${formData.certificado_path ? 'bg-green-100 text-green-600' : 'bg-slate-200 text-slate-500'}`}>
                    ...
                    <div className="flex-1 space-y-1">
                      ...
                      {formData.certificado_info ? (
                        <div className="text-xs text-slate-600 space-y-2 mt-2 p-3 bg-white rounded border border-slate-100">
                          <div>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="pt-4 space-y-4">
                  <div className="flex items-center gap-2 text-blue-600">
                    ...
                    <h4 className="font-bold">Asistente de Migración Fiscal</h4>
                  </div>
                  <p className="text-xs text-slate-500">
                    Si vienes de otro programa, configura aquí tu último número para continuar la serie legalmente.
                  </p>
                  ...
                  <div className={cn("grid grid-cols-1 md:grid-cols-2 gap-4", formData.numeracion_locked && "opacity-60 pointer-events-none")}>
                    <div className="space-y-2">
                      ...
                      />
                      <p className="text-[10px] text-slate-400">La próxima factura será el {(parseInt(formData.correlativo_inicial) || 0) + 1}</p>
                    </div>

                    <div className="space-y-2">
                      <Label>Última Serie (Audit)</Label>
                      <Input
                  ...
                  {/* Fila de Datos Económicos (Audit) */}
                  ...
                  <div className={cn("grid grid-cols-1 gap-4", formData.numeracion_locked && "opacity-60 pointer-events-none")}>
                    <div className="space-y-2">
                      ...
                      >
                        <span className="truncate">{formData.migracion_last_pdf ? "✓ Factura subida" : "Subir última factura..."}</span>
                  ...
                  <div className={cn("p-4 rounded-lg border bg-slate-50 space-y-3", formData.numeracion_locked && "opacity-60")}>
                    <div className="flex items-start gap-3">
                      ...
                      <div className="space-y-1">
                        <Label className="text-xs font-bold leading-none">Aceptación de Responsabilidad Legal</Label>
                        <p className="text-[11px] text-slate-500 leading-tight">
                          Confirmo que el número inicial coincide con mi contabilidad previa. Entiendo que una numeración incorrecta puede acarrear sanciones de la AEAT y eximo a la plataforma de cualquier responsabilidad por duplicidad o saltos en la serie.
                        </p>
                      </div>
                    </div>
