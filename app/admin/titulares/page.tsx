...
export default function TitularesPage() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const res = await authenticatedFetch("/api/admin/titulares");
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = isUpdate ? `/api/admin/titulares/${editing.id}` : "/api/admin/titulares";
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("¿Eliminar este titular? (soft delete, queda histórico)")) return;
    try {
      const res = await authenticatedFetch(`/api/admin/titulares/${id}`, { method: "DELETE" });
      const json = await res.json();
      if (json.ok) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold tracking-tight flex items-center gap-2">
            <Users className="h-6 w-6" /> Titulares y socios
          </h1>
          <p className="text-muted-foreground text-sm">
            Personas vinculadas a la empresa con su régimen de Seguridad Social. Una empresa puede tener varios titulares con regímenes distintos (autónomo + régimen general).
          </p>
        </div>
      ...
      {sumaInvalida && (
        <div className="rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800 flex gap-2 items-start">
          ...
          <span>La suma de participaciones es {totalParticipacion}%. Debería sumar 100% entre todos los titulares activos.</span>
        </div>
      )}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>El régimen SS de cada persona determina si cotiza en RETA o régimen general.</CardDescription>
        </CardHeader>
        <CardContent>
          {items.length === 0 ? (
            <p className="text-sm text-muted-foreground">No hay titulares registrados. Si eres autónomo individual, crea un titular con tu nombre y régimen RETA.</p>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Si es autónomo, se creará automáticamente un registro de empleado vinculado al usuario admin.
            </DialogDescription>
          </DialogHeader>
          {editing && (
